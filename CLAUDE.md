...
- `deploy/` — canonical GPULux Docker Compose and Incus deployment; `deploy/README.md` is normative.
...
| `deploy/docker-compose.yml` or `deploy/gpulux-deploy.sh` changes | Keep `docs/env-vars.md` and `deploy/README.md` synchronized with the canonical GPULux stack, secret-file contract, rollout order and health gates |
