...
- **api is stateless and horizontally scalable.** SSE (queue/TV/judging
  ...
  so any number of `api` replicas behind an external load balancer see every
  event — a client
...
| `api` replica count | Compose / host | 1 | 1 is fine up to ~600 CCU on a reasonably sized box (2+ vCPU). Add a 2nd replica only if you see sustained CPU saturation and place both behind the host's health-aware load balancer. |
| `worker` replica count | Compose / host | 1 | 1 is fine; bump to 2 only if `notification_outbox` depth (query below) climbs during the event instead of draining. |
...
1. **Set the env vars above** in the `.env` files used by Compose a few days
   before the event, not on
...
The stack has no public ingress and no host ports; it uses one Docker network with
...
#### One command: plain Compose on the production host
...
Run the qualification stack directly on the production host from the release
checkout. Do not reuse any production service, network, volume, database,
Valkey, secret, or env file. Set only `RELEASE_IMAGE` and
`QUALIFICATION_ARTIFACT_DIR` in that qualification environment, then run the
same `deploy/qualification/run.sh` from the host shell. The qualification
...
while still managing both with Compose.
