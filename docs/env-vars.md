...
| `LOG_EXPO_PUSH_TICKETS` | container | no (default `false`) | **api and worker**. When `true`, logs redacted Expo push-ticket status, ticket IDs, category, and platform for delivery debugging. Tokens and notification content are never logged. Enable only while investigating push delivery. |
...
| `LOG_EXPO_PUSH_UNSAFE_DEBUG` | container | no (default `false`) | **api and worker**. Explicitly unsafe full-debug mode: logs complete push tokens, message payloads, user IDs, Expo requests/responses, and ticket details. Disabled by default; enable only briefly during local debugging and disable/restart immediately afterward. |
...
| `LOG_EXPO_PUSH_UNSAFE_DEBUG` | container | no (default `false`) | **worker**. Same explicitly unsafe full-debug switch as the API; logs complete push tokens, payloads, requests/responses, user IDs, and ticket details. Enable only briefly while investigating delivery, then disable and restart. |
