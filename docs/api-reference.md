...
| "What changed recently, and why — a specific bug fix's rationale?" | [`audits/implementation-notes.md`](./audits/implementation-notes.md) (historical), [`challenges-devpost.md`](./challenges-devpost.md) |
...
primary email for authenticated, capability, and contextual event mutations;
the application service repeats the check inside the transaction for
submission. Token-based confirmation and decline routes apply the same check
to the target account while holding its user row lock. Account and preparation
operations that do not advance event state opt out explicitly, and the
generated route ledger records the resulting boundary.
...
response to resubmit after the form window closes and auto-accept that next
submission with a fresh confirmation email. Form deletion removes its responses, reviews,
...
Two invariants a new route must not violate: hiding a control in the UI is
never the access check — the server-side policy is; and `public` is an
explicit, audited allowlist entry, never a default a route falls into by
omission.

### Primary-email verification boundary (H1–H3)

The Better Auth sign-in policy intentionally allows an unverified account to
log in. A central pre-handler then protects authenticated, capability, and
contextual mutation routes by default. The policy can mark a route as an
explicit preparation/account exception (`emailVerification: "none"`), while
token routes use `emailVerification: "target"` so the transition service
checks the target account in its locking transaction. In either case, the
policy and service checks use the user's primary `email_verified` field and
return `403` with `email_not_verified` before event state advances.
