...
Three distinct time concepts, deliberately not one field:

1. `event_starts_at` — arrival/doors open → printed on the Wallet pass.
...
- **Header**: the pass date/time = `event_starts_at`, falling back to
  `hacking_starts_at` for deployments that predate `0006` and haven't set it.
