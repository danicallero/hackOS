...
| Logistics | Accreditation and presence, Meals, Activities, Logistics stats | `accredit:scan` or `presence:scan` (Accreditation and presence — a single unified scan/people-finder/hours station), `activity:scan`, `logistics:stats` (each item its own capability — H22-H27 per-station gating) |
