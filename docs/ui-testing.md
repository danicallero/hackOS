...
- **Sign-in needs role-derived `hasEventAccess`**
  (`apps/api/src/modules/identity/role.ts`): a fresh account bounces
  straight back to the form unless it holds an assigned, non-deleted role with
  `event_access = true`. `hasEventAccess` is the canonical API field; an
  accepted application or capability alone no longer grants it.
