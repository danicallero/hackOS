...
- **Admitted-participant eligibility** (`isAdmittedParticipant` in
  `service.ts`) — a thin wrapper around the shared role-derived
  `hasEventAccess` check, reused verbatim rather than reimplemented. An
  account must hold an assigned, non-deleted role with `event_access = true`
  to create, be invited into, or otherwise touch a project this way; an
  application status, capability, or association alone is not an entitlement
  source.
