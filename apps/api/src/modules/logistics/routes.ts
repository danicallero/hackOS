...
export function registerLogisticsRoutes(app: FastifyInstance): void {
  ...
  const scanRateLimit = rateLimitGuard(
    "scan",
    { windowSeconds: config.RATE_LIMIT_SCAN_WINDOW_SECONDS, max: config.RATE_LIMIT_SCAN_MAX },
    keyByUser,
  );
  const mealBatchRateLimit = rateLimitGuard(
    "meal-batch",
    {
      windowSeconds: config.RATE_LIMIT_MEAL_BATCH_WINDOW_SECONDS,
      max: config.RATE_LIMIT_MEAL_BATCH_MAX,
    },
    keyByUser,
  );
  const snapshotRateLimit = rateLimitGuard(
    "snapshot",
    {
      windowSeconds: config.RATE_LIMIT_SNAPSHOT_WINDOW_SECONDS,
      max: config.RATE_LIMIT_SNAPSHOT_MAX,
    },
    keyByUser,
  );
  ...
  typed.get(
    "/api/scanner/snapshot",
    {
      ...
      preHandler: [snapshotRateLimit, logisticsRead],
      schema: {
  ...
  typed.get(
    "/api/public/activities",
    {
      ...
      schema: {
        ...
        description:
          "H47/H48/H59 schedule feed. Everyone else (including anonymous callers) always sees the participant slice, and signed-in mentor or sponsor callers additionally see items tagged for their own audience; a sponsor rep's sponsor audience is additive, so they see the entire participant schedule plus sponsor-tagged items on top, getting owners/contactNote (never the staff-only notes) on the latter. Non-staff responses expose only the audience tags that apply to that caller, keeping client-side audience filters within the caller's actual access. Staff (any authenticated account holding at least one capability) is unconditionally exempt from both the audience filter *and* the visibility/publishAt one — the full run-of-show including drafts and items scheduled to reveal later, each with its notes, owners/contactNote, and its own `visibility` so a staff client can tell a draft apart from a live item (H59 follow-up: previewing a draft on the run-of-show shouldn't require publishing it first).",
  ...
  typed.post(
    "/api/accreditation/check-in",
    {
      ...
      preHandler: [scanRateLimit, accredit, idempotencyGuard],
      schema: {
        body: checkInBody,
        description:
  ...
  typed.post(
    "/api/presence/scan",
    {
      ...
      preHandler: [scanRateLimit, presence, idempotencyGuard],
      schema: {
  ...
  typed.post(
    ...
    {
      ...routeAccess(access.activity),
      preHandler: [mealBatchRateLimit, activity, idempotencyGuard],
      schema: {
