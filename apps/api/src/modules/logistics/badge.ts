...
export async function resolveByBadge(db: Queryable, badgeId: string): Promise<number> {
  const current = await db.query(`SELECT id FROM users WHERE badge_id = $1`, [badgeId]);
  if (current.rows[0]) return current.rows[0].id as number;

  const revoked = await db.query(`SELECT 1 FROM users WHERE $1 = ANY(badge_id_history) LIMIT 1`, [
    badgeId,
  ]);
