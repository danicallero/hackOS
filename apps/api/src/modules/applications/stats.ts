...
export async function applicationStats(
  ...
  const statusCounts = await pool.query(
    `SELECT r.status, count(*)::int AS n FROM application_responses r
     JOIN users u ON u.id = r.user_id AND u.account_state = 'active' AND u.anonymized_at IS NULL
  ...
  const intolerances = await pool.query(
    `SELECT fi.id AS intolerance_id, fi.label,
     ...
     WHERE r.application_id = $1 AND r.status = 'confirmed' AND u.account_state = 'active' AND u.anonymized_at IS NULL
