...
export function registerInviteRoutes(app: FastifyInstance): void {
  ...
  api.get(
    "/api/invites/lookup",
    {
      ...
      // H538: unauthenticated, token-guarded — rate limited per IP against
      // token-enumeration scanning.
      preHandler: rateLimitGuard("invite-lookup", { windowSeconds: 60, max: 30 }, keyByIp),
      schema: {
        description:
          "Inspect an invite token before accepting it (H9/H10) — resolves an email-verification-token invite or an enterprise invite link. Public, unauthenticated; rate limited to 30/min per IP (H538).",
  ...
  api.post(
    "/api/invites/accept",
    {
      ...
      // H538: unauthenticated, token-guarded account creation — rate limited
      // per IP against token brute-forcing.
      preHandler: rateLimitGuard("invite-accept", { windowSeconds: 3600, max: 20 }, keyByIp),
      schema: {
        description:
          "Redeem an invite token and create the invitee's account (H9/H10). Public, unauthenticated; rate limited to 20/hour per IP (H538).",
        body: z.object({
