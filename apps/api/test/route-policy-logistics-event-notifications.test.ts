...
describe("logistics, event and notifications route policy ledger", () => {
  ...
  it("requires explicit metadata for every owned route and records their access classes", () => {
    ...
    expect(declared).toContainEqual({
      ...
      policy: { kind: "capability", capability: "logistics:stats" },
    });
    expect(declared).toContainEqual({
      method: "GET",
      url: "/api/presence/hours/:userId/export.csv",
      policy: { kind: "capability", capability: "logistics:stats" },
    });
    expect(declared).toContainEqual({
      method: "GET",
      url: "/api/audit/actions",
      policy: { kind: "capability", capability: "audit:read" },
    });
    expect(declared).toContainEqual({
      method: "GET",
      url: "/api/audit/:id",
    ...
    expect(declared).toContainEqual({
      method: "POST",
      url: "/api/universities/:id/normalize",
      policy: { kind: "capability", capability: "intolerances:manage" },
    });
    expect(declared).toContainEqual({
      method: "GET",
