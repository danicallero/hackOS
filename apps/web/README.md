...
dark-first zinc-based family. It consumes the Fastify API (Better Auth
included) and follows the same user stories (`plan/historias-hackos.md`) as the
backend, one workstream at a time.

> **The web app is its own deployable service behind the GPULux proxy.** It is
> never served by, or routed together with, the API. See the `web` service in
> `deploy/docker-compose.yml` and the GPULux deployment runbook.
...
src/
  ...
  components/
    ui/                shadcn primitives — generated structure + shared tokens
...
Generated by the shadcn CLI; behavior and structure remain vendored. Add more with:

```sh
pnpm dlx shadcn@latest add <name> -y
```

The maintained exception is the token-backed visual contract: default control
geometry and interaction states in this directory may be aligned with
`globals.css` when a shadcn update would otherwise reintroduce visual drift.
Never add domain behavior here. If a primitive needs a project-specific
composition or behavior variant, wrap it in `components/common/`.
...
scale: `sm` is compact (32 px), the default is 36 px, and `lg` is prominent
(40 px). Icon buttons use the matching `icon-xs` (24 px), `icon-sm`, default,
or `icon-lg` variants. Use the component's `size` prop; do not add `h-*`,
`min-h-*`, `size-*`, or vertical padding to override a control's geometry.
`Textarea` keeps its multiline minimum in the shared textarea token rather than
using a control-height override.
`SelectTrigger size="content"` is the explicit exception for values that must
wrap. `ActionGroup` owns wrapping and the 8 px gap for adjacent actions, while
`IconButton` requires a localized accessible name and keeps icon-only hit areas
consistent.
