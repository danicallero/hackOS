...
export function LocalizedLegalDocument({ copy }: { copy: Record<Language, LegalDocumentCopy> }) {
  ...
  return (
    <>
      ...
      >
        <div className="text-muted-foreground space-y-4 text-pretty leading-7 [&_a]:text-foreground [&_a]:underline [&_a]:underline-offset-4 [&_h2]:!mb-2 [&_h2]:!mt-10 [&_h2]:text-balance [&_h2]:text-xl [&_h2]:font-semibold [&_h2]:text-foreground [&_h2]:sm:text-2xl [&_li]:pl-1 [&_ol]:ml-5 [&_ol]:list-decimal [&_ol]:space-y-2 [&_p]:my-4 [&_strong]:text-foreground [&_ul]:ml-5 [&_ul]:list-disc [&_ul]:space-y-2">
          <ReactMarkdown
            components={{
              a: ({ href, children }) => {
