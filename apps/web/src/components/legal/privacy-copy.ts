...
Puedes iniciar el cierre de la cuenta desde los ajustes («Zona de peligro»). El servidor comprueba el historial operativo en el momento de la solicitud: una cuenta sin registros de presencia ni de participación se elimina por completo, incluidos los datos de solicitudes que se puedan borrar; una cuenta con historial de participación se anonimiza de forma irreversible.

La anonimización revoca el acceso y elimina los identificadores directos, credenciales, entradas, datos de autenticación de Wallet, archivos y la información alimentaria cuando ya no sea necesaria para las operaciones de comida durante el evento. Se crea un nuevo registro aleatorio de participante anónimo sin tabla de consulta ni otra correspondencia con la identidad. Solo pueden permanecer juntos para auditoría y asistencia anónimas: edad, género, universidad, titulación, año de graduación, ciudad de origen y tiempo verificado de presencia en el recinto. Estos campos pueden crear riesgo de reidentificación en grupos muy pequeños, por lo que HackUDC debería agrupar u ocultar combinaciones poco frecuentes al publicar estadísticas. La anonimización impide emitir más adelante justificantes nominales de participación o documentación ECTS. [hackudc@gpul.org](mailto:hackudc@gpul.org) sigue disponible como canal secundario de privacidad y asistencia.
...
Podes iniciar o peche da conta desde os axustes («Zona de perigo»). O servidor comproba o historial operativo no momento da solicitude: unha conta sen rexistros de presenza nin de participación elimínase por completo, incluídos os datos de solicitudes que se poidan borrar; unha conta con historial de participación anonimízase de forma irreversible.

A anonimización revoga o acceso e elimina os identificadores directos, credenciais, entradas, datos de autenticación de Wallet, ficheiros e a información alimentaria cando xa non sexa necesaria para as operacións de comida durante o evento. Créase un novo rexistro aleatorio de participante anónimo sen táboa de consulta nin outra correspondencia coa identidade. Só poden permanecer xuntos para auditoría e asistencia anónimas: idade, xénero, universidade, titulación, ano de graduación, cidade de orixe e tempo verificado de presenza no recinto. Estes campos poden crear risco de reidentificación en grupos moi pequenos, polo que HackUDC debería agrupar ou ocultar combinacións pouco frecuentes ao publicar estatísticas. A anonimización impide emitir máis adiante xustificantes nominais de participación ou documentación ECTS. [hackudc@gpul.org](mailto:hackudc@gpul.org) segue dispoñible como canle secundaria de privacidade e asistencia.
...
You can start account closure from the account settings ("Danger zone"). The server checks the account's operational history at the time of the request: an account with no venue or other participation records is fully deleted, including eligible application data; an account with participation history is irreversibly anonymised instead.

Anonymisation revokes access and removes direct identifiers, credentials, tickets, wallet authentication data, files and dietary information once it is no longer needed for live catering operations. It creates a new random anonymous participant record with no lookup table or other identity mapping. Only the following may remain together for anonymous audit and attendance purposes: age, gender, university, degree, graduation year, origin city and verified venue-presence time. These fields may create re-identification risk in a very small cohort, so HackUDC should aggregate or suppress unusual combinations when publishing statistics. Anonymisation cannot later support named participation proof or ECTS documentation. A mailto link to [hackudc@gpul.org](mailto:hackudc@gpul.org) remains available as a secondary privacy/support channel.
